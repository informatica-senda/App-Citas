...
const RequestScreen = ({ navigation }) => {
  ...
  const setupRequestsListeners = useCallback(() => {
    ...
    try {
      // Primero, obtener todos los usuarios de la misma compañía
      ...
      const unsubscribeUsers = onSnapshot(
        ...
        async (userSnapshot) => {
          ...
          // Para cada usuario, configurar un listener para sus solicitudes no confirmadas
  ...
  const handleDeny = async () => {
    if (selectedRequest) {
      try {
        ...
        Alert.alert("Error", "No se pudo denegar la cita. Inténtalo de nuevo.")
      }
    }
  }
  ...
  // Función para confirmar directamente una cita sin calendario
  const handleDirectConfirm = async () => {
    if (!selectedRequest || !selectedRequest.date) return

    setIsConfirming(true)

    try {
      // Check if the appointment date still doesn't conflict with any other appointments
      ...
      if (hasConflict) {
        Alert.alert(
          ...
          "Ya existe una cita programada para esta fecha y hora. Por favor, programa la cita para otro horario.",
          [
            {
              text: "Programar otro horario",
              onPress: () => {
                setIsConfirming(false)
                setShowActionModal(false)
                // Open the calendar to select another time
                setShowCalendar(true)
              },
            },
            {
            ...
            {selectedRequest.date ? (
              ...
              >
                {isConfirming ? (
                  ...
                  <Text style={styles.confirmButtonText}>Programar</Text>
                )}
              </TouchableOpacity>
            )}
          </View>
