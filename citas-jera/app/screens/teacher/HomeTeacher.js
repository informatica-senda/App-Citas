// HomeTeacher.js - Adapted for web
...
const EmptyScreen = () => {
  return (
    <SafeAreaView style={styles.container}>
      <Header userName="Profesional" screenName="Perfil" />
    </SafeAreaView>
  );
};
      ...
      <View style={[styles.sidebarContainer, !sidebarExpanded && styles.sidebarCollapsed]}>
        ...
        <View style={styles.sidebarContent}>
          ...
          <TouchableOpacity
            style={[
              styles.sidebarItem,
              activeTab === 'Pacientes' && styles.sidebarItemActive,
              !sidebarExpanded && styles.sidebarItemCollapsed,
            ]}
            onPress={() => setActiveTab('Pacientes')}
          >
            <MaterialCommunityIcons
              name={activeTab === 'Pacientes' ? 'account-multiple' : 'account-multiple-outline'}
              size={24}
              color={activeTab === 'Pacientes' ? Colors.PRIMARYCOLOR : '#555'}
            />
            {sidebarExpanded && (
              <Text style={[styles.sidebarItemText, activeTab === 'Pacientes' && styles.sidebarItemTextActive]}>
                Pacientes
              </Text>
            )}
          </TouchableOpacity>
  ...
  const renderContent = () => {
    if (responsive.isDesktop) {
      ...
      return (
        <View style={styles.desktopContentContainer}>
          ...
          {activeTab === 'Pacientes' && <EmployeesScreen />}
        </View>
      );
    }
    ...
    return (
      <Tab.Navigator
        ...
        screenListeners={{
          tabPress: (e) => {
            // Podemos añadir analíticas u otras funciones aquí
          },
        }}
      >
        <Tab.Screen
